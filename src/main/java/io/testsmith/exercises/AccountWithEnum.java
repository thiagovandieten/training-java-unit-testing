...
public class AccountWithEnum {
    ...
    public AccountWithEnum(int balance, int accountNumber, AccountType accountType) {
        ...
        if (this.accountType.equals(AccountType.SAVINGS)) {
            if (amountToWithdraw > this.balance) {
                throw new ArithmeticException(
                        String.format("Cannot withdraw %d from a savings account with balance %d", amountToWithdraw, this.balance));
            }
        }
